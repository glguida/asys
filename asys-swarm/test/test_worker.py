"""Named dispatch remains an ordinary job; owned worlds cannot outlive it."""
...
from asys_swarm.world_process import WorldProcess
...
Service(os.environ["ASYS_RUNTIME_ROOT"],sys.modules[__name__],identity="owned-test-world",channel=os.environ["ASYS_WORLD_CHANNEL"]).serve()
'''
...
class NamedWorkerTests(unittest.TestCase):
    ...
    def swarm(self, *, member_delay=0, turns=1, world=WORLD):
        ...
        config = {'version': 1, 'world': {'command': ['./world.py'], 'timeoutSeconds': 3},
    ...
    def test_named_swarm_is_one_runtime_job_with_owned_world_and_two_member_attempts(self):
        _, _ = self.swarm()
        ...
        with Environment(self.env).register(self.runtime):
            ...
            completed = subprocess.run([sys.executable, str(ROOT / 'asys-runtime/tools/asys-runtime'),
                ...
                env={**os.environ, 'PYTHONPATH': self.environment['PYTHONPATH']}, capture_output=True, text=True, timeout=15)
        ...
        worlds = list((self.job / 'swarm/world').glob('*/state.json'))
        self.assertEqual(len(worlds), 1)
        self.assertEqual(read_json(worlds[0])['status'], 'stopped')

    def test_cancel_stops_world_and_members(self):
        ...
        self.assertTrue(all(read_json(path)['status'] == 'stopped'
                            for path in (self.job / 'swarm/world').glob('*/state.json')))
        self.assertTrue(all(read_json(path)['status'] in {'cancelled', 'interrupted'}
                            for path in (self.job / 'swarm/decisions').glob('*/state.json')))

    def test_two_named_jobs_share_runtime_without_sharing_channels_or_world_processes(self):
    ...
    def test_world_failure_during_member_inference_preserves_committed_world(self):
        ...
        child = self.start(definition)
        self.wait(lambda: list((self.job / 'swarm/decisions').glob('*/state.json')))
        world = next((self.job / 'swarm/world').glob('*/state.json'))
        # This PID is read from the live test-owned world, not a recovery path.
        os.kill(read_json(world)['pid'], 9)
    ...
    def test_worker_crash_cleans_world_and_resumes_completed_checkpoint(self):
        ...
        child.kill()
        child.wait(timeout=5)
        self.wait(lambda: all(read_json(path)['status'] == 'stopped'
                             for path in (self.job / 'swarm/world').glob('*/state.json')))
        ...
        self.assertEqual(len(list((self.job / 'swarm/world').glob('*/state.json'))), 2)
    ...
    def test_dead_world_fails_promptly_and_cleans_descendants(self):
        definition, _ = self.swarm(world='raise RuntimeError("world start failed")\n')
        started = time.monotonic()
        child = self.start(definition)
        self.assertEqual(child.wait(timeout=5), 1)
        self.assertLess(time.monotonic() - started, 3)
        self.assertIn('World executable exited', read_json(self.job / 'result.json')['exception'])

    def test_world_descendants_share_group_and_stop_on_parent_eof(self):
        code = ('import json,os,subprocess,sys,time\nfrom pathlib import Path\n'
                'child=subprocess.Popen([sys.executable,"-c","import time;time.sleep(30)"])\n'
                'Path("pids.json").write_text(json.dumps({"pids":[os.getpid(),child.pid],"group":os.getpgrp()}))\n'
                'time.sleep(30)\n')
        world = WorldProcess(self.job / 'world', [sys.executable, '-c', code], self.environment, self.runtime, 'world-test')
        self.worlds.append(world)
        self.wait(lambda: (self.env / 'pids.json').exists())
        data = read_json(self.env / 'pids.json')
        self.assertEqual(data['group'], os.getpgrp())
        world.close()
        self.wait(lambda: all(not Path(f'/proc/{pid}').exists() for pid in data['pids']))
