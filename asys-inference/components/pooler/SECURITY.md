...
Private Provider traffic uses DComp-owned Unix sockets. Port 8080 is the
component-owned health/catalogue REST API and is published only when this is
the selected outer Provider. That API has no authentication and reveals model
catalogue metadata, so it must not be published to an untrusted network. The
private Provider interface is available only through its DComp socket.
