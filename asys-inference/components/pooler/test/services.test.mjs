...
test("declares one Provider output and one upstream Provider", async () => {
...
test("health and RPC failures expose the same catalogue diagnostic once", async () => {
  ...
  await withPooler({}, async ({ component, provider, healthURL }) => {
    ...
    assert.equal(await checkPoolerHealth({ url: healthURL }), false);
    const health = await (await fetch(healthURL)).json();
    assert.deepEqual(health, { status: "not_ready", message: expected });
  }, invalidConfig, {
...
async function withPooler(overrides, run, config = EXACT_CONFIG, serviceOptions = {}) {
  ...
  try {
    ...
    const upstreamClient = createClient(Provider, createTestTransport(upstreamTarget));
    ...
    httpServer = createProviderHTTPServer({
      component: services.component,
      provider: services.provider,
    });
    const httpAddress = await listenHostAPIServer(httpServer, {
      host: "127.0.0.1",
      port: 0,
    });
    await run({
      ...
      healthURL: `http://127.0.0.1:${httpAddress.port}/health`,
    });
  } finally {
    if (httpServer) await closeHostAPIServer(httpServer).catch(() => {});
